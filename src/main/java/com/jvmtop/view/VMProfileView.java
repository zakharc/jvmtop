...
public class VMProfileView extends AbstractConsoleView {
	...
	public void printView() throws Exception {
		...
		w = width - (1 + 6 + 3 + 9 + 3 + 2);
		for (Iterator<MethodStats> iterator = cpuSampler_.getTop(20).iterator(); iterator.hasNext();) {
			...
			if (!Double.isNaN(wallRatio)) {
				System.out.printf(" %6.2f%% (%9.2fs) %s()%n", wallRatio,
	...
	/**
	 * Shortens a full qualified class name if it exceeds the size. TODO: improve
	 * method to shorten middle packages first, maybe abbreviating the package by
	 * its first character.
	 *
	 * @param fqn
	 * @param method
