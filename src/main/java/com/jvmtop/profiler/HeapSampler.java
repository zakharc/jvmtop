...
public class HeapSampler {
	...
	private static final Pattern HIST_PATTERN = Pattern.compile("\\s+(\\d+):{1}\\s+(\\d+)\\s+(\\d+)\\s+(.+)");
	...
	/**
	 * Returns the human-readable form of the byte.
	 * <p/>
	 * for eg, 1024 bytes would result in 1KiB. Note that, the conversion is done in
	 * <b>IEC system</b>
	 * </p>
	 * that means, 102400 bytes would give 100KiB rather than, 102.4KB. the
	 * converted format is in <b>XX.YYY [suffix]</b>
	 * <p/>
	 * Check https://en.wikipedia.org/wiki/Binary_prefix for more information
	 * <p>
	 *
	 * @param bytes input bytes
	 * @return an array containing human readable form of memory and suffix
	 *         respectively
	 */
	public static String[] toHumanForm(long bytes) {
		...
		for (Entry<Long, String> entry : HeapSampler.binaryPrefixes_IEC.entrySet()) {
			...
			if (amount.intValue() > 0) {
				hForm = new String[] { String.format("%4.3f", amount), entry.getValue() };
			} else {
				break;
			}
		}
	...
	/**
	 * Returns "String" version of human-readable form of the byte.
	 * <p/>
	 * for eg, 1024 bytes would result in 1KiB. Note that, the conversion is done in
	 * <b>IEC system</b>
	 * </p>
	...
	public Set<HeapHistogram> getHistogram(boolean updateDeltas) throws IOException {
		...
		while ((heapLine = data.readLine()) != null) {
			...
			if (matcher.matches()) {
				updatedHist.add(new HeapHistogram(matcher.group(4), matcher.group(2), matcher.group(3)));
			}
		}
	...
	protected void updateDeltas(Set<HeapHistogram> updatedSamples, Set<HeapHistogram> original) {
		for (HeapHistogram updatedSample : updatedSamples) {
			...
			innerloop: for (HeapHistogram originalSample : original) {
...
//                        updatedSample.delta = String.format("%s%-5.3f%%", sign, changePer);
	...
	public boolean dumpHeap(File file) throws IOException {
		...
		final BufferedReader br = new BufferedReader(
				new InputStreamReader(hVm.dumpHeap(file.getAbsolutePath() + ".hprof")));
		String line;
		while ((line = br.readLine()) != null) {
			data.append(line).append("\n");
		}
